...
class LocalDatabase:
    ...
    def get_user_bookings(self, email):
        ...
        try:
            bookings = self._read_file(self.bookings_file)
            user_bookings = [b for b in bookings.values() if b.get('email') == email]
            logger.info(f"Retrieved {len(user_bookings)} bookings for user: {email}")
    ...
    def cancel_booking(self, booking_id, email):
        ...
        try:
            ...
            if booking.get('email') != email:
                logger.warning(f"Unauthorized booking cancellation attempt: {booking_id}")
                return False
